...
def triggers_str(text):
    ...
    if res == 1:
        # Action triggers
        subs = ["find", "search", "lookfor", "lookingfor", "seek", "seeking"]
        found = False
        for sub in subs:
            if sub in t:
                found = True
                break
        if found:
            subs = ["alien", "extraterrestrial", "lifeform", "babyyoda"]
            for sub in subs:
                if sub in t:
                    res = 2
                    break
    if res != 0:
        print(f"DEBUG: Found trigger pattern {res}")
    return res, text


class LaikaQuery(Plugin):
    """
    ...
    def process(self, input, **kwargs):
        ...
        if self.listening:
            ...
            if trig == 0:  # no pattern
                return
            if not len(input):
                self.listening = False  # waiting for prompt
                return
        else:
            self.listening = True  # prompt has arrived, back to listening for triggers
        ...
        self.output(self.stream, LaikaQuery.OutputStream)
        ...
        for token in self.stream:
            ...
            self.output(token, LaikaQuery.OutputToken)
            ...
            if last_space >= 0:
                self.output(words[:last_space + 1], LaikaQuery.OutputWords)
        ...
        if len(words) > 0:
            self.output(words, LaikaQuery.OutputWords)
        ...
        self.output(bot_reply.text, LaikaQuery.OutputFinal)
